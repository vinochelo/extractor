...
): Promise<{ success: true; data: RetentionRecord } | { success: false; error: string }> {
  try {
    ...
    const extractedData = await extractRetentionDataFromPDF({ pdfDataUri: fileAsDataUrl });
    ...
    const retencionesCollection = firestoreAdmin.collection('users').doc(userId).collection('retenciones');
    const docRef = await retencionesCollection.add(retentionRecord);

    const docSnapshot = await docRef.get();
    const finalData = { id: docRef.id, ...docSnapshot.data() } as RetentionRecord;
